...
return_type_t UART::Sender(const char *data, uint8_t receiver, bool response) 
{
    ...
    busy=true;
    ESP_LOGI(UART_TAG,"CPU1 GIDEN >> %s",data);
                    ...
                    if (event.size==header_size) 
                      {
                        ...
                        if (event.size>=header_size)
                        {
                            ...
                            if (hd->flag.paket_type==PAKET_PONG)
                             {
                              //printf("pong geldi\n");
                              if (mthis->PING_LED!=-1) gpio_set_level(mthis->PING_LED, 0);
                              mthis->ping_stat=true;
                              mthis->ping_error_counter = 0;
                ...
                {     
                    ...
                    if (pos == -1) {
                      ...
                      if (pos>0)
                      {
                        ...
                        //if(hd->sender==mthis->ping_device) 
                        mthis->ping_timer_restart();
                        mthis->ping_error_counter = 0;
