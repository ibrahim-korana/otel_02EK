...
static const char *KONT_TAG = "WATER";

void Water::timer_callback(void* arg)
{
    Water *mthis = (Water *)arg;
    ...
    mthis->write_status();
    ESP_ERROR_CHECK(esp_event_post(FUNCTION_OUT_EVENTS, ROOM_ACTION, (void *)mthis, sizeof(Water), portMAX_DELAY));
}

void Water::set_motorlu(bool drm)
{
    ...
    free(cc);
    tim_start(sr);
    ESP_ERROR_CHECK(esp_event_post(FUNCTION_OUT_EVENTS, ROOM_ACTION, (void *)this, sizeof(Water), portMAX_DELAY));
    ESP_LOGI(KONT_TAG,"WATER PORT ACTION");
}

void Water::set_motorsuz(bool drm)
{
    Base_Port *target = port_head_handle;
    while (target) {
        if (target->type == PORT_OUTPORT) 
...
{      
    ...
    {   
        ...
        if (chg)
        {
            ...
            ESP_ERROR_CHECK(esp_event_post(FUNCTION_REMOTE_EVENTS, ROOM_ACTION, (void *)this, sizeof(Water), portMAX_DELAY));
        }      
    }
}
...
void Water::in_handler(void* handler_args, esp_event_base_t base, int32_t id, void* event_data)
{
    home_status_t *st = (home_status_t *) event_data;
    Water *con = (Water *) handler_args;
    uint8_t dev_id = 0;
...
//Alarm mesajları
void Water::alarm_handler(void* handler_args, esp_event_base_t base, int32_t id, void* event_data)
{
    home_virtual_t *st = (home_virtual_t *) event_data;
    Water *con = (Water *) handler_args;
    if (st->stat) {
        //alarm deprem mesajı alındı suyu kapat
...
void Water::tim_start(uint8_t tm)
{
...
/*
       Water, kendisine tanımlanan roleyi açıp kapatan bir objedir. 
       olası tanımlama:
       {
            "name":"water",
            "uname":"water",
            ...
            "hardware": {
                "location" : "local",
                "port": [
                    {"pin": 5, "pcf":1, "name":"Sıcak Su","type":"PORT_OUTPORT"}
                ]
            }
        }
            ...
            Objeye, sistem Motorlu ise isimleri ON ve OFF olan en az 2,
               ...
               water objesi motorlu ve onoff olmak üzere 2 tiptir. Cıkış tipinin
            ...
            water oda rolesine baglı olarak tanımlanmış ise Odanın kapanması ile 
        kapatılır, odaya fiziksel olarak girilmesi ile açılır. 
            sistemin yazılım ile açılıp kapatılması mümkündür

            Yangın veya deprem durumunda su kapatılır.
